...
const BestsellersReport = () => {
  ...
  const [position, setPosition] = useState({ x: 0, y: 0 });
    ...
    const columns: Array<GridColDef> = [
        ...
        {
            field: 'ORDER_QTY', headerName: 'ORDER QTY', width: 170, type: 'string', resizable: true, align: 'right', headerAlign: 'right',
            renderCell: (params: GridValueGetterParams) => {
                return <p>{params.value}</p>
            }
        },
    ...
    const getBestsellers = async (fromdate:string,todate:string) => {
        const response = await axios({
            url: `http://localhost:5035/admin/reports/bestsellers?FROM=${fromdate}&TO=${todate}`,
            method: 'GET',
    ...
    <div className='report-sales-container'>
        <div className='report-sales-container__filters'>
            ...
            <div className='report-sales-container__filters__filter'>
                <button
                    onClick={(e) => { getBestsellers(fromDate,toDate) }}
